...
* A litmitation for the **Outcomes based on Launch Date** analysis is that their isn't more specified grouping for the individual campaigns. You can see which months have the most successful campaigns, but we don't know when those campaigns start (beginning of the month, middle of the month, etc) and what the goal and pledged amounts were for each campaign.  Without that information, you have to be careful about what definitive conclusions you draw.  For example,  it's possible that a higher percentage of campaigns that started in May had a fundraising goal of under $5,000, as compared to other months, which led to a higher rate of  *successful* outcomes than you'd expect if the range of campaign goal amounts were evenly distributed for each month.  
